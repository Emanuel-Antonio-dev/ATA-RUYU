...
class PrismaGraduationRepositories implements IGraduationsRepositories
{
    ...
    {
        return this.prisma.graduation.findFirst({where: {athleteId,status: { in: ['NOT_ELIGIBLE', 'ELIGIBLE'] }}})
    }
    async getAllGraduations(academyId: string): Promise<any[]> {
        return await this.prisma.graduation.findMany({where:{academyId: academyId},
            include:{
                athlete:{
                    select:{
                        id: true,
