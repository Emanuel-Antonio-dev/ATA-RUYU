...
class PrismaAthelesRepositories implements IAtheleRepositories
{
    ...
    async updateAthlete(id: string, datas: Partial<UpdateAthleteDto>, tx?: Omit<Prisma.TransactionClient, "$transaction">): Promise<any> {
        const client = tx ?? this.prisma
        return await client.athlete.update({where:{id: id}, data: datas})
    }
    async getAllAtheles(filters: {page:number;limit:number;}): Promise<any> {
